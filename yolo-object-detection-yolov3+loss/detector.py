...
def Write(x, results):
    ...
    img = results[int(x[0])]
    cls = int(x[-1])
    c_path = "/home/saman/python-projects/yolo-object-detection/pallete"
    colors = pkl.load(open(c_path, "rb"))
    ...
    cv2.rectangle(img, c1, c2, color, -1)
    cv2.putText(
        img, label, (c1[0], c1[1] + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN,
        1, [225, 255, 255], 1
    )
    return img
...
def arg_parse():
    ...
    parser.add_argument(
        "--bs", dest='bs', help="batch size", default=1
    )
    parser.add_argument(
        "--confidence", dest="confidence", default=0.5,
        help="Object Confidence to filter predictions"
    )
    parser.add_argument(
        "--nms_thresh", dest="nms_thresh",
        help="NMS Threshhold", default=0.4
    )
    parser.add_argument("--cfg", dest='cfgfile', help="Config file",
                        default=path+"cfg/yolov3_3.cfg", type=str)
    parser.add_argument("--weights", dest='weightsfile', help="weightsfile",
                        default=path+"/yolov3.weights", type=str)
...
num_classes = 3
path = '/home/saman/python-projects/yolo-object-detection/data/logo.names'
classes = load_classes(path)

print("...Loading Network!")
model = Darknet(args.cfgfile)
# model.load_weights(args.weightsfile)
model.load_state_dict(torch.load("/home/saman/python-projects/yolo-object-detection/weights/best_model_weights.pt", map_location='cpu'))
